...
export type AttendanceScanKioskProps = {
  previewScan: (raw: string) => Promise<AttendancePreviewResponse>
  confirmScan: (
    raw: string,
    intent: Extract<AttendanceIntent, 'check_in' | 'check_out'>
  ) => Promise<{ payload: AttendanceConfirmResponse; detail?: string }>
  /** When set, shows a back control that calls this handler. */
  onBack?: () => void
  backLabel?: string
  titleHint?: string
}

export function AttendanceScanKiosk({
  previewScan,
  confirmScan,
  onBack,
  backLabel = 'Kembali',
  titleHint = 'Arahkan kamera ke kartu staf',
}: AttendanceScanKioskProps) {
  ...
  const handleScan = useCallback(
    async (raw: string) => {
      ...
      setProcessing(true)
      try {
        const preview = await previewScan(raw)
        ...
        const { payload, detail } = await confirmScan(raw, intent)
    ...
    [confirmScan, previewScan, showScanNotice]
  )
        ...
        <div className="relative h-full w-full">
          <div className="absolute top-4 left-4 right-4 z-10 flex items-center justify-between gap-2">
            {onBack ? (
              ...
              <p className="rounded-md bg-black/55 px-3 py-2 text-sm font-medium text-white/90">
                {titleHint}
              </p>
            )}
            <Button
